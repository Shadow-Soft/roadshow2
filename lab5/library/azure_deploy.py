...
def get_ip_dict(ip):
    return dict(name=ip.name,
                ...
                dns_settings=get_dns_settings_dict(ip.dns_settings))

def get_dns_settings_dict(dns_settings):

  if dns_settings is None:
    return dict()

  return dict(fqdn=dns_settings.fqdn,
              reverse_fqdn=dns_settings.reverse_fqdn,
              domain_name_label=dns_settings.domain_name_label)
